...
class LoraMoeLayer(BaseTunerLayer):
    ...
    other_param_names = ("r", "lora_alpha", "scaling", "lora_moe_dropout")
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights):
        ...
        if adapter_name in self.lora_moe_A.keys():
            if init_lora_weights is True:
                ...
                nn.init.normal_(self.lora_moe_A[adapter_name].weight, std=1 / self.r[adapter_name])
            else:
    ...
    def olora_init(self, adapter_name):
        ...
        self.lora_moe_A[adapter_name].weight.data = Rr.contiguous()
        self.lora_moe_B[adapter_name].weight.data = Qr.contiguous()

        weight_tensor.data -= scale_factor * self.lora_moe_B[adapter_name].weight @ self.lora_moe_A[adapter_name].weight
    ...
    def pissa_init(self, adapter_name, init_lora_weights):
        ...
        lora_moe_A = torch.diag(torch.sqrt(Sr)) @ Uhr
        lora_moe_B = Vr @ torch.diag(torch.sqrt(Sr))
        self.lora_moe_A[adapter_name].weight.data = lora_moe_A
        self.lora_moe_B[adapter_name].weight.data = lora_moe_B
        weight = weight.data - self.scaling[adapter_name] * lora_moe_B @ lora_moe_A
    ...
    def loftq_init(self, adapter_name):
        ...
        qweight, lora_moe_A, lora_moe_B = loftq_init(weight, **kwargs)
        if adapter_name in self.lora_moe_A.keys():
            # initialize A the same way as the default for nn.Linear and B to zero
            self.lora_moe_A[adapter_name].weight.data = lora_moe_A
            self.lora_moe_B[adapter_name].weight.data = lora_moe_B
        if adapter_name in self.lora_moe_embedding_A.keys():
            # initialize a the same way as the default for nn.linear and b to zero
            self.lora_moe_embedding_A[adapter_name].weight.data = lora_moe_A
            self.lora_moe_embedding_B[adapter_name].weight.data = lora_moe_B
    ...
    def dora_init(self, adapter_name: str) -> None:
        ...
        lora_moe_A = self.lora_moe_A[adapter_name].weight
        lora_moe_B = self.lora_moe_B[adapter_name].weight
        place_on_cpu = self.ephemeral_gpu_offload and (lora_moe_A.device.type == "cpu" or lora_moe_B.device.type == "cpu")
        if self.ephemeral_gpu_offload:
            if lora_moe_A.device.type == "cuda":
                lora_moe_B = lora_moe_B.to(lora_moe_A.device)
            else:
                if lora_moe_B.device.type != "cuda":
                    lora_moe_B = lora_moe_B.to("cuda")
                lora_moe_A = lora_moe_A.to(lora_moe_B.device)
        scaling = self.scaling[adapter_name]
        dora_layer.update_layer(
            base_layer=self.get_base_layer(), lora_moe_A=lora_moe_A, lora_moe_B=lora_moe_B, scaling=scaling, place_on_cpu=place_on_cpu
        )
    ...
    def _mixed_batch_forward(
        ...
        for i, active_adapter in enumerate(unique_adapters):
            ...
            # layer output
            sub_batch = x[sub_batch_indices_list[i]].to(lora_moe_A.weight.dtype)
            lora_output = lora_moe_B(lora_moe_A(dropout(sub_batch))) * scaling
...
class LoraLinear(nn.Module, LoraMoeLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.multiple_loras:
            ...
            self.w_gate = nn.Linear(self.in_features, num_experts) 
    ...
    def forward(self, x: torch.Tensor, inputs_embeds: torch.Tensor, attention_mask: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            # print("self.loss_coef", self.loss_coef)
            # print("self.freeze_gate", self.freeze_gate)
            if self.multiple_loras:
                if self.token:
                    # all-token start
                    bs, sl, _ = x.size()
                    ...
                    clean_logits = self.w_gate(self.ln(prompt_embeds)) 
                ...
                for active_adapter in self.active_adapters:
                    ...
                    if not self.use_dora[active_adapter]:
                        result = result + lora_moe_B(lora_moe_A(dropout(x))) * scaling
                    else:
                        x = dropout(x)
                        result = result + self.lora_magnitude_vector[active_adapter](
                            x,
                            lora_moe_A=lora_moe_A,
                            lora_moe_B=lora_moe_B,
                            scaling=scaling,
                            base_layer=self.get_base_layer(),
                        )
...
class LoraLinear_noGate(nn.Module, LoraMoeLayer):
    # Lora implemented in a dense layer
    def __init__(
        self,
        base_layer,
    ...
    def forward(self, x: torch.Tensor, gates: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        self._check_forward_args(x, *args, **kwargs)
        adapter_names = kwargs.pop("adapter_names", None)
...
class Embedding(nn.Module, LoraMoeLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        LoraMoeLayer.__init__(self, base_layer)
...
class Conv2d(nn.Module, LoraMoeLayer):
...
def dispatch_default(
    ...
    **kwargs,
    ...
    assert target_name is not None, "target name cannot be None"
    ...
    if isinstance(target_base_layer, torch.nn.Embedding):
        ...
        embedding_kwargs.update(moe_config.loftq_config)
        new_module = Embedding(target, adapter_name, **embedding_kwargs)
    elif isinstance(target_base_layer, torch.nn.Conv2d):
        kwargs.update(moe_config.loftq_config)
        ...
        kwargs.update(moe_config.loftq_config)
        # if "gate_proj" in target_name or "q_proj" in target_name or "k_proj" in target_name or "v_proj" in target_name or "o_proj" in target_name:
        if adapter_name == "default":
            if "q_proj" in target_name or "k_proj" in target_name or "v_proj" in target_name:
            # print(":linear", target_name)
                new_module = Linear(target, adapter_name, **kwargs)
        else:
            if "gate_proj" in target_name or "o_proj" in target_name:
                # print("{}:LoraLinear".format(target_name))
                ...
                # print("{}:LoraLinear_noGate".format(target_name))
        ...
        # new_module = LoraLinear(target, adapter_name, **kwargs)
        ...
        kwargs.update(moe_config.loftq_config)
        new_module = LoraLinear(target, adapter_name, is_target_conv_1d_layer=True, **kwargs)
