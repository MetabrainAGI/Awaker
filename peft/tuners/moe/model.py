...
from .layer import Conv2d, LoraMoeLayer, dispatch_default
...
class MoeModel(BaseTuner):
    ...
    Args:
        ...
        config ([`MoeConfig`]): The configuration of the Lora model.
    ...
    Example:
        ...
        >>> from peft import LoraModel, MoeConfig
        ...
        >>> import torch
        >>> import transformers
        >>> from peft import MoeConfig, PeftModel, get_peft_model, prepare_model_for_kbit_training
    ...
    **Attributes**:
        ...
        - **peft_config** ([`MoeConfig`]): The configuration of the Lora model.
    """
    ...
    @staticmethod
    def _check_target_module_exists(moe_config, key):
        return check_target_module_exists(moe_config, key)

    def _prepare_model(self, moe_config: MoeConfig, model: nn.Module):
        r"""
        ...
        """
        if moe_config.layer_replication:
            replicate_layers(model, moe_config.layer_replication)
    ...
    def _create_and_replace(
        ...
        pattern_keys = list(chain(moe_config.rank_pattern.keys(), moe_config.alpha_pattern.keys()))
        target_name_key = next(filter(lambda key: re.match(rf".*\.{key}$", current_key), pattern_keys), current_key)
        r = moe_config.rank_pattern.get(target_name_key, moe_config.r)
        alpha = moe_config.alpha_pattern.get(target_name_key, moe_config.lora_alpha)

        kwargs = {
            "r": r,
            "lora_alpha": alpha,
            "lora_dropout": moe_config.lora_dropout,
            "fan_in_fan_out": moe_config.fan_in_fan_out,
            "init_lora_weights": moe_config.init_lora_weights,
            "use_rslora": moe_config.use_rslora,
            "use_dora": moe_config.use_dora,
            "ephemeral_gpu_offload": moe_config.runtime_config.ephemeral_gpu_offload,
        ...
        if isinstance(target, LoraMoeLayer) and not isinstance(target, AdaLoraLayer):
            target.update_layer(
                adapter_name,
                r,
                lora_alpha=alpha,
            ...
            new_module = self._create_new_module(moe_config, adapter_name, target, **kwargs)
    ...
    def _mark_only_adapters_as_trainable(self, model: nn.Module) -> None:
        for n, p in model.named_parameters():
            ...
            if ("moe_" not in n) and ("w_gate" not in n):
                p.requires_grad = False
        ...
        for active_adapter in self.active_adapters:
            ...
            if bias == "all":
                ...
                for m in model.modules():
                    if isinstance(m, LoraMoeLayer) and hasattr(m, "bias") and m.bias is not None:
    ...
    def _create_new_module(moe_config, adapter_name, target, **kwargs):
        ...
        if moe_config._custom_modules:
            ...
            def dynamic_dispatch_func(target, adapter_name, moe_config, **kwargs):
                new_module = None
                ...
                for key, custom_cls in moe_config._custom_modules.items():
        ...
        new_module = None
        new_module= dispatch_default(target, adapter_name, moe_config=moe_config, **kwargs)
        # for dispatcher in dispatchers:
        #     new_module = dispatcher(target, adapter_name, moe_config=moe_config, **kwargs)
    ...
    def add_weighted_adapter(
        ...
        for key in key_list:
            ...
            if isinstance(target, LoraMoeLayer):
                if adapter_name in target.lora_moe_A:
                    target_lora_moe_A = target.lora_moe_A[adapter_name].weight
                    target_lora_moe_B = target.lora_moe_B[adapter_name].weight
                elif adapter_name in target.lora_moe_embedding_A:
                    target_lora_moe_A = target.lora_moe_embedding_A[adapter_name]
                    target_lora_moe_B = target.lora_moe_embedding_B[adapter_name]
                else:
                    continue

                target_lora_moe_A.data = target_lora_moe_A.data * 0.0
                target_lora_moe_B.data = target_lora_moe_B.data * 0.0
                if combination_type == "cat":
                    loras_moe_A, loras_moe_B = [], []
                    for adapter, weight in zip(adapters, weights):
                        if adapter in target.lora_moe_A:
                            current_adapter_lora_moe_A = target.lora_moe_A[adapter].weight
                            current_adapter_lora_moe_B = target.lora_moe_B[adapter].weight
                        elif adapter in target.lora_moe_embedding_A:
                            current_adapter_lora_moe_A = target.lora_moe_embedding_A[adapter]
                            current_adapter_lora_moe_B = target.lora_moe_embedding_B[adapter]
                        else:
                            continue
                        loras_moe_A.append(current_adapter_lora_moe_A.data * weight * target.scaling[adapter])
                        loras_moe_B.append(current_adapter_lora_moe_B.data)

                    if len(loras_moe_A) == 0:
                        raise ValueError("No matching LoRAs found. Please raise an issue on GitHub.")
                    loras_moe_A = torch.cat(loras_moe_A, dim=0)
                    loras_moe_B = torch.cat(loras_moe_B, dim=1)
                    target_lora_moe_A.data[: loras_moe_A.shape[0], :] = loras_moe_A
                    target_lora_moe_B.data[:, : loras_moe_B.shape[1]] = loras_moe_B
                    ...
                    target_lora_moe_A.data, target_lora_moe_B.data = self._svd_generalized_task_arithmetic_weighted_adapter(
                        combination_type,
                        adapters,
                        weights,
                        new_rank,
                    ...
                    target_lora_moe_A.data, target_lora_moe_B.data = self._generalized_task_arithmetic_weighted_adapter(
    ...
    def _svd_generalized_task_arithmetic_weighted_adapter(
        ...
        valid_adapters = []
        valid_weights = []
        is_embedding = any(adapter in target.lora_moe_embedding_A for adapter in adapters)
        for adapter, weight in zip(adapters, weights):
            if adapter in target.lora_moe_A or adapter in target.lora_moe_embedding_A:
        ...
        if conv2d:
            U = U.reshape(target_lora_moe_B.data.shape)
            Vh = Vh.reshape(target_lora_moe_A.data.shape)
        return Vh, U
    ...
    def _generalized_task_arithmetic_weighted_adapter(
        ...
        for adapter, weight in zip(adapters, weights):
            ...
            lora_moe_A_deltas.append(current_adapter_lora_moe_A.data)
            lora_moe_B_deltas.append(current_adapter_lora_moe_B.data)
        valid_weights = torch.tensor(valid_weights).to(lora_moe_A_deltas[0].device)
        lora_moe_deltas = [lora_moe_A_deltas, lora_moe_B_deltas]
        dtype = lora_moe_A_deltas[0].dtype
        for i, task_tensors in enumerate(lora_moe_deltas):
            if combination_type == "linear":
                lora_moe_deltas[i] = task_arithmetic(task_tensors, valid_weights)
            elif combination_type == "ties":
                lora_moe_deltas[i] = ties(task_tensors, valid_weights, density, majority_sign_method)
            elif combination_type == "dare_linear":
                lora_moe_deltas[i] = dare_linear(task_tensors, valid_weights, density)
            elif combination_type == "dare_ties":
                lora_moe_deltas[i] = dare_ties(task_tensors, valid_weights, density, majority_sign_method)
            elif combination_type == "magnitude_prune":
                lora_moe_deltas[i] = magnitude_prune(task_tensors, valid_weights, density)
            else:
                raise ValueError("Invalid combination type")
        lora_moe_deltas = [delta.to(dtype) for delta in lora_moe_deltas]
        return lora_moe_deltas
