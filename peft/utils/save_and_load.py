...
def get_peft_model_state_dict(
    ...
    if config.peft_type in (PeftType.LORA, PeftType.ADALORA):
        ...
        bias = config.bias
        if bias == "none":
            print("Save while lora-bias is none...")
            to_return = {k: state_dict[k] for k in state_dict if "moe" in k or "w_gate" in k}
        elif bias == "all":
            print("Save while lora-bias is all...")
            to_return = {k: state_dict[k] for k in state_dict if "moe"  in k or "bias" in k or 'w_gate' in k}
        elif bias == "lora_only": # 没有修改
            print("Save while lora-bias is lora_only...")
            to_return = {}
            for k in state_dict:
                if "lora_" in k:
        ...
        to_return = {k: v for k, v in to_return.items() if (("moe" in k and adapter_name in k) or ("bias" in k) or ("w_gate" in k))}
    ...
    # elif config.peft_type == PeftType.MOE:
    #     print("全部存下来")
